...
{
    public sealed class EquipmentSaveBridge : MonoBehaviour
    {
        ...
        public void SaveSlot(int slot)
        {
            ...
            var save = new EquipmentSaveFile { SlotIndex = Mathf.Max(0, slot) };

            foreach (var item in tracker.Equipped)
            {
                if (item != null)
                {
                    save.Equipped.Add(item);
                }
            }
        ...
        public void LoadSlot(int slot)
        {
            ...
            var save = JsonUtility.FromJson<EquipmentSaveFile>(File.ReadAllText(path));
            if (save == null)
            {
                return;
            }
            ...
            foreach (var item in save.Equipped)
            {
                tracker.ImportRecord(item);
            }
        }

        public static string GetPath(int slot)
        {
            string dir = Path.Combine(Application.persistentDataPath, "equipment");
            Directory.CreateDirectory(dir);
            return Path.Combine(dir, $"slot_{Mathf.Max(0, slot)}_equipment.json");
        }
    }
}
